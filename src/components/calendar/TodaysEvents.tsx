...
export function TodaysEvents() {
  ...
  const getTimeStatus = (event: TodayEvent) => {
    ...
    if (isFuture(event.startTime)) {
      ...
      return { status: 'ongoing', text: `${minutesLeft} min left`, color: 'text-generator-darkGreen' };
    }
  };
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-generator-darkGreen">Today's Events</h3>
              </div>
              <span className="text-xs text-generator-darkGreen font-medium">
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    timeStatus.status === 'ongoing' && "border-generator-darkGreen/50 bg-generator-darkGreen/10",
                    timeStatus.status === 'upcoming' && index === 0 && "border-generator-darkGreen/30 bg-gray-100"
                  )}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h4 className="font-semibold text-generator-darkGreen">{todayEvent.event.title}</h4>
                      ...
                      {todayEvent.event.shortBlurb && (
                        <p className="text-xs text-gray-600 mb-2">{todayEvent.event.shortBlurb}</p>
                      )}
                    ...
                    {todayEvent.event.registrationUrl && timeStatus.status !== 'ended' && (
                      <a
                        ...
                        className="flex-shrink-0 px-3 py-1 bg-generator-darkGreen text-white text-xs rounded-md hover:opacity-90 transition-opacity"
                      >
                        Register
                      </a>
                    )}
